...
export async function recordInfectionControlTaskUpdate(input: { role?: string; actor: string; taskIds: string[]; priority?: "high" | "medium" | "low"; status?: InfectionControlTaskStatus; comment?: string; writeAudit: (entry: { actor: string; role: string; tool: string; outcome: string; detail: string }) => Promise<unknown>; updateTasks: (input: { taskIds: string[]; priority?: "high" | "medium" | "low"; status?: InfectionControlTaskStatus; lastComment?: string; updatedBy: string }) => Promise<unknown> }) {
  validateInfectionControlTaskUpdate(input);
  const updatedTasks = await input.updateTasks({ taskIds: input.taskIds, priority: input.priority, status: input.status, lastComment: input.comment, updatedBy: input.actor });
