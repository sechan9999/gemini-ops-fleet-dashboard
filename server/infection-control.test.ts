...
describe("infection-control overview", () => {
  ...
  it("gates bulk task updates and records one durable audit entry", async () => {
    const auditRows: Array<{ actor: string; role: string; tool: string; outcome: string; detail: string }> = [];
    const result = await recordInfectionControlTaskUpdate({ role: "medical_director", actor: "Dr. HK Chun", taskIds: ["ipc-precaution-review", "ipc-surface-verification"], priority: "medium", status: "in_progress", comment: "Reviewed with ward lead", writeAudit: async row => { auditRows.push(row); }, updateTasks: async input => input });
