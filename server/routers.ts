...
export const appRouter = router({
  ...
  fleet: router({
    ...
    infectionControlCommentSummary: protectedProcedure.input(z.object({ range: z.enum(["daily", "weekly"]), comments: z.array(z.object({ taskId: z.string(), comment: z.string().max(500), actor: z.string().max(160), role: z.string().max(80), createdAt: z.string() })).max(200) })).mutation(async ({ input }) => {
      ...
      const classify = (comment: string): Category => { const text = comment.toLowerCase(); return text.includes("verify") || text.includes("review") || text.includes("evidence") ? "verification" : text.includes("ppe") || text.includes("supply") || text.includes("resource") ? "resource" : text.includes("train") || text.includes("refresher") ? "training" : text.includes("cover") || text.includes("staff") || text.includes("ward") ? "coverage" : "other"; };
      const assignments = input.comments.map((entry) => ({ taskId: entry.taskId, category: classify(entry.comment), createdAt: entry.createdAt }));
      const fallbackCounts = Object.fromEntries(categories.map((category) => [category, 0])) as Record<Category, number>;
      assignments.forEach((entry) => { fallbackCounts[entry.category] += 1; });
      const bucketKey = (value: string) => { const date = new Date(value); if (Number.isNaN(date.getTime())) return "unknown"; if (input.range === "daily") return date.toISOString().slice(0, 10); const day = date.getUTCDay(); const diff = day === 0 ? -6 : 1 - day; date.setUTCDate(date.getUTCDate() + diff); return date.toISOString().slice(0, 10); };
      const trendMap = new Map<string, Record<Category, number>>();
      assignments.forEach((entry) => { const key = bucketKey(entry.createdAt); const bucket = trendMap.get(key) || Object.fromEntries(categories.map((category) => [category, 0])) as Record<Category, number>; bucket[entry.category] += 1; trendMap.set(key, bucket); });
      const trend = Array.from(trendMap.entries()).sort(([left], [right]) => left.localeCompare(right)).map(([dateKey, counts]) => ({ dateKey, ...counts }));
      ...
      try {
        ...
        const content = response.choices?.[0]?.message?.content;
        if (typeof content === "string") { const parsed = JSON.parse(content) as { summary?: string; categories?: Array<{ name: string; count: number; note: string }> }; if (parsed.summary?.trim()) return { summary: parsed.summary.trim(), categories: parsed.categories || [], assignments, trend, source: "gemini" as const }; }
      } catch { /* deterministic fallback below */ }
      return { summary: fallback, categories: categories.filter((category) => fallbackCounts[category] > 0).map((category) => ({ name: category, count: fallbackCounts[category], note: "Keyword-based operational grouping" })), assignments, trend, source: "deterministic_fallback" as const };
    }),
